 ...
 */
public class AdUrlViewpagerUtil {

    private Context context;
    private ViewPager viewPager;
    private ImageViewUrlPagerAdapter mimageViewUrlPagerAdapter;
     ...
     * @param urls
     */
    public AdUrlViewpagerUtil(Context context, ViewPager viewPager, ArrayList urls) {
     ...
     * @param urls
     */
    public AdUrlViewpagerUtil(Context context, ViewPager viewPager, LinearLayout dotlayout, int dotsize,
                              int dotoffset, List<String> urls,int position) {
        this.context = context;
        this.viewPager = viewPager;
    ...
    private void initAdimgs(ArrayList urls)
    {
    ...
    private void setImg(int length, ArrayList urls)
    {
        if (urls.size() > 0) {
            imgsize = length;
            for (int i = 0; i < length; i++) {
            ...
            Glide.with(context) .load(urls.get(urls.size() - 1)).into(mImageViews[0]);//urls.size() - 1
            Glide.with(context) .load(urls.get(0)).into(mImageViews[length - 1]);
        }
    }

    public void initVps(final int num)
    {
        initAdimgs((ArrayList) urls);
        initDots(urls.size());
        mimageViewUrlPagerAdapter = new ImageViewUrlPagerAdapter(context, mImageViews);
        viewPager.setAdapter(mimageViewUrlPagerAdapter);
